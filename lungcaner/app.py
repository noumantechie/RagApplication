...
try:
    ...
    st.error(
        "API Key not found in secrets. Please check the Streamlit Secrets configuration.")
    st.stop()
...
df['combined'] = df.astype(str).apply(' '.join, axis=1)
...
def retrieve_relevant_rows(query, top_n=3):
    ...
    similarities = cosine_similarity(
        query_embedding, vector_database)  # Compute similarities
    top_indices = np.argsort(similarities[0])[
        ::-1][:top_n]  # Get top_n indices
